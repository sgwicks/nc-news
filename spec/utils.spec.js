...
describe.only('formatComments', () => {
  ...
  it('Formats the created_by date into a JS Date', () => {
    const comments = [{
      body:
        "Oh, I've got compassion running out of my nose, pal! I'm the Sultan of Sentiment!",
      belongs_to: "They're not exactly dogs, are they?",
    ...
    const timestamp2 = new Date(1479818163389)

    expect(formatComments(comments, articleRef)[0].created_at).to.be.an.instanceOf(Date)
    expect(formatComments(comments, articleRef)[0].created_at).to.eql(timestamp1)
    expect(formatComments(comments, articleRef)[1].created_at).to.eql(timestamp2)
  })
